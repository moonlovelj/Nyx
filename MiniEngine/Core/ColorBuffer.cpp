...
void ColorBuffer::GenerateMipMaps(CommandContext& BaseContext)
{
    ...
    if (ArraySize > 1 && m_ArraySRVHandle.ptr != D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
    {
        Context.SetDynamicDescriptor(1, 0, m_ArraySRVHandle);
    }
    else
    {
        ...
        for (uint32_t TopMip = 0; TopMip < m_NumMipMaps; )
        {
            ...
            if (ArraySize > 1)
            {
				if (m_Format == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB)
					Context.SetPipelineState(Graphics::g_GenerateMipsArrayGammaPSO[NonPowerOfTwo]);
				else
					Context.SetPipelineState(Graphics::g_GenerateMipsArrayLinearPSO[NonPowerOfTwo]);

				__declspec(align(16)) struct ArrayConstants
				{
					uint32_t SliceIndex;
				};

                ArrayConstants cbData;
                cbData.SliceIndex = SliceIndex;
                Context.SetDynamicConstantBufferView(3, sizeof(ArrayConstants), &cbData);
            }
            else
            {
                if (m_Format == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB)
